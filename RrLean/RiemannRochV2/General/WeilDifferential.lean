...
/-! ## Non-Degeneracy Definitions (Cycle 316)

The crux of Serre duality is proving the pairing is non-degenerate:

  ∀ f ∈ L(D), f ≠ 0 → ∃ ω ∈ Ω(K-D), ⟨f, ω⟩ ≠ 0

We define non-degeneracy properties for the general pairing L(D) × Ω(E) → k.

### Key Definitions

* `PairingNondegenerateLeft` - For f ≠ 0 in L(D), ∃ ω in Ω(E) with ⟨f, ω⟩ ≠ 0
* `PairingNondegenerateRight` - For ω ≠ 0 in Ω(E), ∃ f in L(D) with ⟨f, ω⟩ ≠ 0
* `PairingPerfect` - Both left and right non-degeneracy
* `dim_eq_of_perfect_pairing` - Perfect pairing ⟹ dim L(D) = dim Ω(E)

### Strategy for Proving Non-Degeneracy (Cycle 317+)
...
/-- Left non-degeneracy: for any nonzero f ∈ L(D), there exists ω ∈ Ω(E)
such that ⟨f, ω⟩ ≠ 0.

This says the pairing "separates" elements of L(D). -/
def PairingNondegenerateLeft (D E : DivisorV2 R) : Prop :=
  ∀ f : RRSpace_proj k R K D, (f : K) ≠ 0 →
    ∃ ω : DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E,
      serrePairingGeneral D E f ω ≠ 0

/-- Right non-degeneracy: for any nonzero ω ∈ Ω(E), there exists f ∈ L(D)
such that ⟨f, ω⟩ ≠ 0.

This says the pairing "separates" elements of Ω(E). -/
...
/-- Alternative formulation: left non-degeneracy via kernel of the linear map.

The pairing is left non-degenerate iff the map L(D) →ₗ[k] (Ω(E) →ₗ[k] k)
has trivial kernel. -/
def PairingNondegenerateLeft' (D E : DivisorV2 R) : Prop :=
  LinearMap.ker (serrePairingGeneral (k := k) (K_infty := K_infty) D E) = ⊥

/-- Alternative formulation: right non-degeneracy via kernel.

The pairing is right non-degenerate iff for each nonzero ω,
the map f ↦ ⟨f, ω⟩ is nonzero. -/
def PairingNondegenerateRight' (D E : DivisorV2 R) : Prop :=
  ∀ ω : DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E,
    (ω : WeilDifferential k R K K_infty) ≠ 0 →
    serrePairingGeneral D E |>.flip ω ≠ 0

/-- Left non-degeneracy is equivalent to the kernel formulation. -/
lemma pairingNondegenerateLeft_iff_kernel (D E : DivisorV2 R) :
    PairingNondegenerateLeft (k := k) (R := R) (K := K) (K_infty := K_infty) D E ↔
    PairingNondegenerateLeft' (k := k) (R := R) (K := K) (K_infty := K_infty) D E := by
  constructor
  · -- Left nondeg → kernel is trivial
    intro hnd
    rw [PairingNondegenerateLeft'] at *
    ext f
    simp only [LinearMap.mem_ker, Submodule.mem_bot]
    constructor
    · intro hf
      -- If serrePairingGeneral D E f = 0, show f = 0
      by_contra hne
      -- f ≠ 0 as submodule element means (f : K) ≠ 0
      have hfK : (f : K) ≠ 0 := by
        intro h
        apply hne
        ext
        ...
        exact h
      obtain ⟨ω, hω⟩ := hnd f hfK
      -- But hf says ∀ ω, pairing f ω = 0
      have : serrePairingGeneral D E f ω = 0 := by
    ...
    · intro hf
      rw [hf]
      simp only [map_zero]
  · -- Kernel trivial → left nondeg
    intro hker f hfne
    rw [PairingNondegenerateLeft'] at hker
    by_contra hall
    push_neg at hall
    -- ∀ ω, pairing f ω = 0 means f ∈ ker
    have hfker : f ∈ LinearMap.ker (serrePairingGeneral (k := k) (K_infty := K_infty) D E) := by
      rw [LinearMap.mem_ker]
      ext ω
      exact hall ω
    rw [hker] at hfker
    simp only [Submodule.mem_bot] at hfker
    -- So f = 0 as submodule element
    have : (f : K) = 0 := by
      rw [hfker]
      simp only [Submodule.coe_zero]
    exact hfne this
...
When the pairing L(D) × Ω(E) → k is perfect and both spaces are finite-dimensional,
they have the same dimension. This is a general fact about non-degenerate bilinear forms.
-/

/-- A non-degenerate bilinear pairing between finite-dimensional spaces
induces an injection from one space to the dual of the other.

In particular, if both sides are non-degenerate, the dimensions are equal. -/
theorem dim_le_of_nondeg_left (D E : DivisorV2 R)
    [FiniteDimensional k (RRSpace_proj k R K D)]
    [FiniteDimensional k (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E)]
    (hnd : PairingNondegenerateLeft (k := k) (R := R) (K := K) (K_infty := K_infty) D E) :
    Module.finrank k (RRSpace_proj k R K D) ≤
    Module.finrank k (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E) := by
  -- The map L(D) →ₗ[k] (Ω(E) →ₗ[k] k) is injective
  have hinj : Function.Injective (serrePairingGeneral (k := k) (K_infty := K_infty) D E) := by
    rw [← LinearMap.ker_eq_bot]
    rwa [← pairingNondegenerateLeft_iff_kernel]
  -- The target is the dual space Ω(E)*, which has same dimension as Ω(E)
  have hdual : Module.finrank k
      (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E →ₗ[k] k) =
      Module.finrank k (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E) := by
    exact Module.finrank_linearMap_self k _ k
  -- Injective linear map implies dimension inequality
  calc Module.finrank k (RRSpace_proj k R K D)
      ≤ Module.finrank k
          (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E →ₗ[k] k) :=
        LinearMap.finrank_le_finrank_of_injective hinj
    _ = Module.finrank k
          (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E) := hdual

/-- Symmetric result: right non-degeneracy implies dimension inequality in the other direction. -/
theorem dim_le_of_nondeg_right (D E : DivisorV2 R)
    [FiniteDimensional k (RRSpace_proj k R K D)]
    [FiniteDimensional k (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E)]
    (hnd : PairingNondegenerateRight (k := k) (R := R) (K := K) (K_infty := K_infty) D E) :
    Module.finrank k (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E) ≤
    Module.finrank k (RRSpace_proj k R K D) := by
  -- The flipped map Ω(E) →ₗ[k] (L(D) →ₗ[k] k) has trivial kernel
  have hinj : Function.Injective ((serrePairingGeneral (k := k) (K_infty := K_infty) D E).flip) := by
    rw [← LinearMap.ker_eq_bot]
    ext ω
    simp only [LinearMap.mem_ker, Submodule.mem_bot]
    constructor
    · intro hω
      by_contra hne
      have hneW : (ω : WeilDifferential k R K K_infty) ≠ 0 := by
        intro h
        apply hne
        ext
        simp only [Submodule.coe_zero]
        exact h
      obtain ⟨f, hf⟩ := hnd ω hneW
      have : serrePairingGeneral D E f ω = 0 := by
        have := LinearMap.ext_iff.mp hω f
        simp only [LinearMap.flip_apply, LinearMap.zero_apply] at this
        exact this
      exact hf this
    · intro hω
      rw [hω]
      simp only [map_zero]
  have hdual : Module.finrank k (RRSpace_proj k R K D →ₗ[k] k) =
      Module.finrank k (RRSpace_proj k R K D) :=
    Module.finrank_linearMap_self k _ k
  calc Module.finrank k (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E)
      ≤ Module.finrank k (RRSpace_proj k R K D →ₗ[k] k) :=
        LinearMap.finrank_le_finrank_of_injective hinj
    _ = Module.finrank k (RRSpace_proj k R K D) := hdual

/-- **Main theorem**: A perfect pairing implies equal dimensions.

If the pairing L(D) × Ω(E) → k is non-degenerate on both sides and
both spaces are finite-dimensional, then dim L(D) = dim Ω(E). -/
theorem dim_eq_of_perfect_pairing (D E : DivisorV2 R)
    [FiniteDimensional k (RRSpace_proj k R K D)]
    [FiniteDimensional k (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E)]
  ...
  · exact dim_le_of_nondeg_left D E hperf.1
  · exact dim_le_of_nondeg_right D E hperf.2

end NonDegeneracy

/-! ## Canonical Divisor and Serre Duality (Cycle 316)

The canonical divisor K is a divisor class such that:
1. deg(K) = 2g - 2 (where g is the genus)
2. Ω(0) ≅ L(K) as k-vector spaces
3. The Serre pairing L(D) × Ω(K - D) → k is perfect

We define a typeclass capturing these properties, which can be instantiated
for specific curve families (P¹, elliptic curves, general curves).
-/
...
/-- The canonical divisor class data for a curve.

This packages:
- `canonical`: a representative divisor in the canonical class
- `genus`: the genus g of the curve (dim Ω(0) = g)
- `deg_canonical`: deg(K) = 2g - 2
- `serre_duality`: the pairing L(D) × Ω(K - D) → k is perfect for all D

This is the key structure needed to state and prove Riemann-Roch. -/
class HasCanonicalDivisor where
  ...
  /-- Ω(0) has dimension equal to the genus. -/
  dim_omega_zero :
    Module.finrank k (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) 0) = genus
  /-- Finite dimensionality of L(D) for all D. -/
  finrank_rrspace (D : DivisorV2 R) : FiniteDimensional k (RRSpace_proj k R K D)
  /-- Finite dimensionality of Ω(E) for all E. -/
  finrank_omega (E : DivisorV2 R) :
    FiniteDimensional k (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E)
  /-- The Serre duality pairing is perfect for all divisors D. -/
  serre_duality (D : DivisorV2 R) :
    PairingPerfect (k := k) (R := R) (K := K) (K_infty := K_infty) D (canonical - D)

variable [HasCanonicalDivisor k R K K_infty]

/-- The canonical divisor. -/
abbrev canonicalDivisor : DivisorV2 R := HasCanonicalDivisor.canonical (k := k) (K_infty := K_infty)

/-- The genus. -/
abbrev curveGenus : ℕ := HasCanonicalDivisor.genus (k := k) (R := R) (K := K) (K_infty := K_infty)

/-- Convenient notation for ℓ(D) = dim L(D). -/
abbrev ell (D : DivisorV2 R) : ℕ :=
  @Module.finrank k (RRSpace_proj k R K D) _ _ (HasCanonicalDivisor.finrank_rrspace D)

/-- Convenient notation for dim Ω(E). -/
abbrev dimOmega (E : DivisorV2 R) : ℕ :=
  @Module.finrank k
    (DivisorDifferentials (k := k) (R := R) (K := K) (K_infty := K_infty) E) _ _
    (HasCanonicalDivisor.finrank_omega E)

/-- **Serre Duality**: ℓ(D) = dim Ω(K - D).

This is the dimension equality that comes from the perfect pairing. -/
theorem serre_duality_dim (D : DivisorV2 R) :
    ell (k := k) (K_infty := K_infty) D =
    dimOmega (k := k) (K_infty := K_infty) (canonicalDivisor (k := k) (K_infty := K_infty) - D) := by
  haveI := HasCanonicalDivisor.finrank_rrspace (k := k) (K_infty := K_infty) D
  haveI := HasCanonicalDivisor.finrank_omega (k := k) (K_infty := K_infty)
      (canonicalDivisor (k := k) (K_infty := K_infty) - D)
  exact dim_eq_of_perfect_pairing D _ (HasCanonicalDivisor.serre_duality D)

/-! ### Riemann-Roch from Serre Duality

With the canonical divisor infrastructure in place, Riemann-Roch becomes:

  ℓ(D) - ℓ(K - D) = deg(D) + 1 - g

The key steps:
1. Serre duality: ℓ(K - D) = dim Ω(D)  [from serre_duality applied to K - D]
2. Rewrite: ℓ(D) - dim Ω(D) = deg(D) + 1 - g

The equality ℓ(D) - dim Ω(D) = deg(D) + 1 - g is the Riemann-Roch theorem
when we identify dim Ω(D) with the "index of specialty" i(D).

For the full Riemann-Roch:
  ℓ(D) - ℓ(K - D) = deg(D) + 1 - g

We need to connect dim Ω(D) to ℓ(K - D).
-/

/-- Serre duality applied to K - D gives: ℓ(K - D) = dim Ω(D).

This uses the fact that (K - (K - D)) = D. -/
theorem serre_duality_swap (D : DivisorV2 R) :
    ell (k := k) (K_infty := K_infty) (canonicalDivisor (k := k) (K_infty := K_infty) - D) =
    dimOmega (k := k) (K_infty := K_infty) D := by
  have h := serre_duality_dim (k := k) (K_infty := K_infty)
      (canonicalDivisor (k := k) (K_infty := K_infty) - D)
  -- Simplify K - (K - D) = D
  simp only [sub_sub_cancel] at h
  exact h

/-- **Riemann-Roch Theorem** (abstract form).

  ℓ(D) - ℓ(K - D) = deg(D) + 1 - g

where K is the canonical divisor and g is the genus.

This follows from Serre duality and the relationship between
the Euler characteristic and the degree. -/
theorem riemann_roch_abstract (D : DivisorV2 R)
    (hRR : ∀ D' : DivisorV2 R,
      (ell (k := k) (K_infty := K_infty) D' : ℤ) -
      (dimOmega (k := k) (K_infty := K_infty) D' : ℤ) = D'.deg + 1 - curveGenus k R K K_infty) :
    (ell (k := k) (K_infty := K_infty) D : ℤ) -
    (ell (k := k) (K_infty := K_infty) (canonicalDivisor (k := k) (K_infty := K_infty) - D) : ℤ) =
    D.deg + 1 - curveGenus k R K K_infty := by
  rw [serre_duality_swap]
  exact hRR D
